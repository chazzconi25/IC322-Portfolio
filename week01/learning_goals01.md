...
- Data and telephone traffic is separated by a DSLAM at a telco Central Office (CO) to be sent over the internet or telephone network.
...
- Endusers submit data via the hybrid fiber coax. End users use a cable modem to transfer data to coax. Neighborhoods connected by coax cables send data to fiber nodes which in turn send data via fiber to the cable head end (CO).
- The cable head end connects to the internet via a CMTS located which is similar to a DSLAM for cable in that it multiplexes signals and sends them to the COs edge router.
...
- Fiber is provided to homes via direct fiber, Active Optical Networks (AONs) and Passive Optical Networks (PONs). The latter two use splitters to split one optical connection from the neighborhood for smaller neighborhoods.
There are several different implementations of fiber. The simplest is direct fiber which runs an individual fiber cable to each home from the CO. This was improved with Active Optical Networks (AON) and Passive Optical Networks(PON). AONs are essentially switched ethernet. PONs have a Optical Line Terminator (OLT) at the CO and an Optical Network Terminator (ONT) at each home. These devices convert optical to wired signals at endpoints. At each neighborhood an Optical Splitter is used to split signals from the CO and fed to ONTs at each home.
...
Frequency Division Multiplexing splits a single medium into multiple frequency bands. In this form of channel multiplexing each node has a specific frequency they are assigned to communicate on and they have the full throughput of the link within that frequency. One downside to this is the number of nodes that can communicate is limited by the number of frequency bands you have available.

#### TDM - Time Division Multiplexing

Similar to FDM TDM divides up a medium into sections where each node has full throughput of the link. In this situation time is divided into frames of fixed duration and each node has a fixed number of slots. During their slot a node can transmit but must wait till their slot comes back to transmit after transmitting.
...
The internet consists of multiple layers that each preform a function to deliver data from one place to another. The definition of place is complicated when it comes to the internet so layers are used to define a different role in moving data. In the five layer internet protocol stack there is Application, Transport, Network, Link, and Physical. Each one of these layers preforms a job essentially unaware of the other layers. In this system all of the layers are highly interconnected to preform the job of transporting data. One example would be that in the application layer packets are transferred from one application to another using port numbers. In this layer the only important information is what port number is attached to the data and what application does it need to go do/ be received by within the host. Outside of that the application layer assumes any host, even millions of miles apart with hundreds of core routers between them, is basically directly connected to another. Conversely, The link layer **only** operates outside of individual hosts and views multiple hosts simply as a router with endpoints, similar to how application sees applications within a host. Both are critical to delivering data accross the internet but do not rely on one another to preform their individual function.
